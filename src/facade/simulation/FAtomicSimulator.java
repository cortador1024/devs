...
import static facade.simulation.FSimulator.STATE_INITIAL;
import static facade.simulation.FSimulator.STATE_PAUSE;
import static facade.simulation.FSimulator.STATE_SIMULATING;
...
public class FAtomicSimulator implements FSimulator {
  ...
  public void run() { 
    simulator. simulate ( Integer.MAX_VALUE );
  }
  ...
  public void setRTMultiplier ( double factor ) {
    simulator.setRTMultiplier(factor);
  }
  
  public double getRTMultiplier () { 
    return simulator.getRTMultiplier();
  }
  
  public double getTimeOfLastEvent () {
    return simulator.getTL();
  }
      ...
      */
      ...
      public synchronized void reset () {
        if ( ( 
          ...
          throw new FIllegalSimulatorStateException ( formatErrorMsg("Can only [Reset] from state: {Initial} | {Pause} | {End}." ) );
        }
        initialize();
        stopRequested    = false;
        suspendRequested = false;
       ...
       */
      public synchronized void simulate ( int n ) {
        if ( n <= 0 ) {
          throw new FIllegalSimulatorParameterException ( String. format ( "Cannot [Simulate], number of iterations must be > 0: %s", n ) );
        }  
        boolean flag = currentState == STATE_INITIAL || currentState == STATE_PAUSE;
        if ( ! flag ) {
          throw new FIllegalSimulatorStateException ( formatErrorMsg ( "Can only [Simulate] from state: {Initial} | {Pause}." ) );
        }
      ...
      public synchronized void requestPause () {
        boolean flag = currentState == STATE_SIMULATING || currentState == STATE_PAUSE;
        if ( ! flag ) {
          throw new FIllegalSimulatorStateException ( formatErrorMsg("Can only [Pause] from state: {Simulating} | {Pause}." ) );
        }
        numIter = 0;
      }
