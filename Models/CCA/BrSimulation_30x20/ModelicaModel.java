...
public class ModelicaModel {
	...
	public ModelicaModel(int x, int y) {
		...
		try {
			if (System.getProperty("os.name").startsWith("Mac")) {
				if (Param.barrestin) {
					path = "Models/Modelica/FMU/Mac/BrSim_30x20.fmu";
				} else {
					path = "Models/Modelica/FMU/Mac/BrSim.fmu";
				}
			} else if (System.getProperty("os.name").startsWith("Win")) {
				path = "Models/Modelica/FMU/Win/BrSim_30x20.fmu";
	...
	public void execute(double startTime, double stopTime, double stepSize) {
		if(simulation == null) {
			System.out.println("\n" + errorMSG + "\n");
			throw new IllegalStateException("\n FMU file not found for " + System.getProperty("os.name") + "\n");		
		}
