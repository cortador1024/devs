...
public class FunctionKIB extends CCAKIBModel {
	...
	private ModelicaModel Modelica;
	...
	private double FMUCoumputingTime = 0.0;
	...
	public FunctionKIB(String name, MapRegion mapR_A, MapRegion mapR_B, double newLocationX_A, double newLocationY_A,
		...
		Modelica = new ModelicaModel(width2, height2);
				...
				for (int j = 0; j < initialStatus[0].length; j++) {
					if (initialStatus[i][j] == "CXCR4") {
						if (Param.barrestin) {
							...
							Modelica.setC4(i, j, 0);
							Modelica.setC4i(i, j, 0);
							Modelica.setR4(i, j, 702061);
							Modelica.setR4i(i, j, 321043);
							Modelica.setR4B(i, j, 32495.6);
							Modelica.setC4B(i, j, 0);
							Modelica.setC4Bii(i, j, 0);
							Modelica.setB4(i, j, 467504);
							Modelica.setCXCL12i(i, j, 0);
						} else {

							Modelica.setC4(i, j, 0);
							Modelica.setC4i(i, j, 0);
							Modelica.setR4(i, j, 64545.5);
							Modelica.setR4i(i, j, 645454);
							Modelica.setCXCL12i(i, j, 0);
						}
						...
						if (Param.barrestin) {
							...
							Modelica.setC7(i, j, 0);
							Modelica.setC7i(i, j, 0);
							Modelica.setR7(i, j, 4758830);
							Modelica.setR7i(i, j, 96768.2);
							Modelica.setR7B(i, j, 322569);
							Modelica.setC7B(i, j, 0);
							Modelica.setR7Bii(i, j, 967430);
							Modelica.setC7Bii(i, j, 0);
							Modelica.setB7(i, j, 80662.9);
							Modelica.setCXCL12i(i, j, 0);
						} else {
							Modelica.setC7(i, j, 0);
							Modelica.setC7i(i, j, 0);
							Modelica.setR7(i, j, 1450250);
							Modelica.setR7i(i, j, 4349750);
							Modelica.setCXCL12i(i, j, 0);
						}
					} else {
						if (Param.barrestin) {
							...
							Modelica.setC4(i, j, 0);
							Modelica.setC4i(i, j, 0);
							Modelica.setR4(i, j, 0);
							Modelica.setR4i(i, j, 0);
							Modelica.setR4B(i, j, 0);
							Modelica.setC4B(i, j, 0);
							Modelica.setC4Bii(i, j, 0);
							Modelica.setB4(i, j, 0);
							Modelica.setC7(i, j, 0);
							Modelica.setC7i(i, j, 0);
							Modelica.setR7(i, j, 0);
							Modelica.setR7i(i, j, 0);
							Modelica.setR7B(i, j, 0);
							Modelica.setC7B(i, j, 0);
							Modelica.setC7Bii(i, j, 0);
							Modelica.setC7Bii(i, j, 0);
							Modelica.setB7(i, j, 0);
							Modelica.setCXCL12i(i, j, 0);
						} else {
							Modelica.setC4(i, j, 0);
							Modelica.setC4i(i, j, 0);
...
//		}
	...
	public void deltext(double e, message x) {
				...
				if (value != null) {
					try {
						...
						if (value.getInput() == "X4") {
							Modelica.moveX4(value);

						} else if (value.getInput() == "X7") {
							Modelica.moveX7(value);

						} else if (value.getInput() == "L12") {
							Modelica.moveL12(value);
		...
		//Calculating the FMU time
		
		long FMUstartTime = System.currentTimeMillis();		
		
		Modelica.execute(360);
				
		long FMUendTime = System.currentTimeMillis();
		
		FMUCoumputingTime = (FMUCoumputingTime + FMUendTime - FMUstartTime);
		
		System.out.println("Accumulated FMU Computing Time (seconds): " + FMUCoumputingTime/1000); 
...
//		content X4Migration = makeContent("AvgX4Migration", new entity(Modelica.getAverageMigration() + ""));
//
//		m.add(X4Migration);
//
//		content ConcAbsGrad = makeContent("ConcAbsGrad", new entity(Modelica.computeConcAbsGrad() + ""));
//
//		m.add(ConcAbsGrad);
	...
	private void DataTransformMacro2Micro(int i, int j, double v) {
		Modelica.accumulateValue(i, j, v);
...
//		String[] attractDirection = {"outSW", "outS", "outSE", "outW", "Stay", "outE", "outNW", "outN", "outNE"};
//		
//		double[] neighborValue = {(j + 1 < height1) ? Value2Macro(i, j + 1) : 0.0,
//				(i - 1 >= 0 && j + 1 < height1) ? Value2Macro(i - 1, j + 1) : 0.0,
//				(i + 1 < width1 && j + 1 < height1) ? Value2Macro(i + 1, j + 1) : 0.0,
//				(i - 1 >= 0) ? Value2Macro(i - 1, j) : 0.0, Value2Macro(i, j),
//				(i + 1 < width1) ? Value2Macro(i + 1, j) : 0.0,
//				(i - 1 >= 0 && j - 1 >= 0) ? Value2Macro(i - 1, j - 1) : 0.0,
//				(j - 1 >= 0) ? Value2Macro(i, j - 1) : 0.0,
//				(i + 1 < width1 && j - 1 >= 0) ? Value2Macro(i + 1, j - 1) : 0.0};
	...
	private double getNumReceptors(int i, int j) {
		double X4Receptors = Modelica.getC4(i, j) + Modelica.getC4B(i, j) + Modelica.getR4(i, j)
				+ Modelica.getR4B(i, j);
...
//		}
		...
		for (int j = 0; j < neighbors; j++) {
			double cxcl12Ratio = cxcl12conc[j] / KD;
			double denominator = (cxcl12Ratio + inhibitorRatio + 1) * 9;

			numReceptorOccupied[j] = (cxcl12Ratio / denominator) * numReceptors; // the denominator takes into account
																					// the division by 9

			if (numReceptorOccupied[j] < numReceptorOccupied[min]) {
				min = j;
			}
		}
		...
		// (sensAdjust)
		for (int j = 0; j < neighbors; j++) {
			prob[j] = numReceptorOccupied[j] - numReceptorOccupied[min] + sensAdjust;
		}
